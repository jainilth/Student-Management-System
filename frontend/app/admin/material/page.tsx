...
import { GetAllMaterials, DeleteMaterial } from "@/service/material.service";
...
export default async function MaterialListPage() {
  ...
  async function removeRecord(formData: FormData) {
    "use server";
    const result = await DeleteMaterial(Number(formData.get("materialId")));
    if (!result?.error) revalidatePath("/admin/material");
  }
  return (
    <section className="space-y-7">
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="border-b border-slate-100 px-5 py-4">
          <h2 className="font-semibold text-slate-900">Materials directory</h2>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[720px] text-left text-sm">
              <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                <tr>
                  <th className="px-5 py-3 font-medium">MaterialId</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {records.map((record: RecordData) => (
                  <tr key={record.materialId} className="hover:bg-emerald-50/30">
                    ...
                    <td className="px-5 py-4 text-slate-600">
                      {String(record.uploadedByUserName || record.uploadedBy || "-")}
                    </td>
                    <td className="px-5 py-4 text-slate-600">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-end gap-3">
                        <Link
                          href={"/admin/material/edit/" + record.materialId}
