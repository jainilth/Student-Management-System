...
export default async function StudentSemesterListPage() {
  const response = await GetAllStudentSemesters();
  ...
  async function removeRecord(formData: FormData) {
    "use server";
    const result = await DeleteStudentSemester(
      Number(formData.get("studentSemesterId")),
    );
    if (!result?.error) revalidatePath("/admin/student-semester");
  }
  return (
      ...
      <header className="flex flex-col gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">
            Manage student semesters records.
          </p>
        </div>
        <Link
          href="/admin/student-semester/create"
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[720px] text-left text-sm">
              <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                <tr>
                  <th className="px-5 py-3 font-medium">StudentSemesterId</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {records.map((record: RecordData) => (
                  <tr
                    key={record.studentSemesterId}
                    className="hover:bg-emerald-50/30"
                  >
                    <td className="px-5 py-4 font-medium text-slate-900">
                      {record.studentSemesterId}
                    </td>
                      ...
                      <div className="flex justify-end gap-3">
                        ...
                        <AdminDeleteForm action={removeRecord}>
                          <input
                            ...
                            value={record.studentSemesterId}
                          />
                          <button
                            type="submit"
