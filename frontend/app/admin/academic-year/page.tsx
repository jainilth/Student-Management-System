...
export default async function AcademicYearListPage() {
  ...
  async function removeRecord(formData: FormData) {
    ...
    const result = await DeleteAcademicYear(
      Number(formData.get("academicYearId")),
    );
    if (!result?.error) revalidatePath("/admin/academic-year");
  }
  return (
      ...
      <header className="flex flex-col gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">
            Manage academic years records.
          </p>
        </div>
        <Link
          href="/admin/academic-year/create"
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[720px] text-left text-sm">
              <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                <tr>
                  <th className="px-5 py-3 font-medium">AcademicYearId</th>
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-end gap-3">
                        <Link
                          href={
                            "/admin/academic-year/edit/" + record.academicYearId
                          }
