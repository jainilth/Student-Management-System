...
export default async function DepartmentListPage() {
  ...
  async function removeRecord(formData: FormData) {
    "use server";
    const result = await DeleteDepartment(Number(formData.get("departmentId")));
    if (!result?.error) revalidatePath("/admin/department");
  }
  return (
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[720px] text-left text-sm">
              <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                <tr>
                  <th className="px-5 py-3 font-medium">DepartmentId</th>
