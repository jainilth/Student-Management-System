...
export default async function AcademicProgramListPage() {
  const response = await GetAllAcademicPrograms();
  ...
  async function removeRecord(formData: FormData) {
    "use server";
    const result = await DeleteAcademicProgram(
      Number(formData.get("programId")),
    );
    if (!result?.error) revalidatePath("/admin/academic-program");
  }
  return (
      ...
      <header className="flex flex-col gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">
            Manage academic programs records.
          </p>
        </div>
        <Link
          href="/admin/academic-program/create"
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[720px] text-left text-sm">
              <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                <tr>
                  <th className="px-5 py-3 font-medium">ProgramId</th>
                  <th className="px-5 py-3 font-medium">Program name</th>
                  <th className="px-5 py-3 font-medium">Program code</th>
                  <th className="px-5 py-3 font-medium">Department</th>
                  <th className="px-5 py-3 font-medium">Duration years</th>
                  <th className="px-5 py-3 font-medium">Total semesters</th>
                  <th className="px-5 py-3 text-right font-medium">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {records.map((record: RecordData) => (
                  <tr key={record.programId} className="hover:bg-emerald-50/30">
                    <td className="px-5 py-4 font-medium text-slate-900">
                      {record.programId}
                    </td>
                    <td className="px-5 py-4 text-slate-600">
                      {String(record.programName ?? "-")}
                    </td>
                    <td className="px-5 py-4 text-slate-600">
                      {String(record.programCode ?? "-")}
                    </td>
                    <td className="px-5 py-4 text-slate-600">
                      {String(record.departmentName || record.departmentId || "-")}
                    </td>
                    <td className="px-5 py-4 text-slate-600">
                      {String(record.durationYears ?? "-")}
                    </td>
                    <td className="px-5 py-4 text-slate-600">
                      {String(record.totalSemesters ?? "-")}
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex justify-end gap-3">
                        <Link
                          href={
                            "/admin/academic-program/edit/" + record.programId
                          }
